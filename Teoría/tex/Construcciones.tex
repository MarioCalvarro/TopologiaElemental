...
\begin{figure}[H]
    \centering
    \incfig[0.7]{rectángulo-a-cilindro-por-imagen-directa.}
...
\begin{figure}[H]
    \centering    
        ...
        \node(X) {$X$};
        \node(Y) [right of=X] {$Y$};
        \node(Z) [below of=X] {$\faktor{X}{\sim}$};
        \draw[->](X) to node {$f$}(Y);
        \draw[->](X) to node [left] {$p$}(Z);
        \draw[->](Z) to node [right=0.2] {$\overline{f}$}(Y);
        \end{tikzpicture}  
        \begin{tikzpicture}[node distance=2cm, auto]
        \node(x) {$x$};
...
\begin{itemize}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[node distance=2cm, auto]
            ...
            \node(Y) [right of=X] {$C \subset \mathbb{R}^3$};
