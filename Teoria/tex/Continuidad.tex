...
\begin{itemize}
    ...
    \[
        \overline{f\left( A \right)} \cerr Y \xRightarrow{3)} \underbrace{f^{-1}\overline{f\left( A \right)}}_{\subset f^{-1}f\left( A \right) \supset A} \subset X \Rightarrow \overline{A} \subset f^{-1}\overline{f\left( A \right)} \Rightarrow f\left( \overline{A} \right) \subset \overline{f\left( A \right)} 
    \]
    \item $5 \Rightarrow 4)$
    \begin{gather*}
...
\begin{enumerate}
    ...
    \begin{demo}
        \begin{itemize}
            ...
            \item $\forall U \stackrel{\text{ab.}}{\subset } X: \underbrace{j^{-1}\left( U \right)}_{U \cap Z} \ab Z$
        \end{itemize}
    \end{demo}
    ...
    \begin{demo}
        ...
        \[
        \begin{rcases}
            x \in W^x: \exists G \ab W^x \subset E^x: G = A \cap E^x \subset X\\ 
            x \in E^x: \exists B \ab E^x \subset X 
...
\begin{demo}
\begin{enumerate}
    \item Sea $F \cerr X \Rightarrow X\setminus F \ab X \xRightarrow{ f\text{ ab.}} \underbrace{f\left( X\setminus F \right)}_{= Y\setminus f\left( F \right) \text{(biy.)}} \ab X \Rightarrow f\left( F \right) \cerr Y \Rightarrow f$ cerrada.

    \item Sea $F \cerr X \xRightarrow{f \text{cerr.}} \underbrace{f\left( F \right)}_{\stackrel[\text{biy.}]{}{=} \left( f^{-1} \right)^{-1} \left( F \right)} \cerr Y \Rightarrow f^{-1}$ continua.

    \item Sea $U \ab X \xRightarrow{f^{-1} \text{cont.}} \underbrace{\left( f^{-1} \right) ^{-1} \left( U \right) }_{\stackrel[\text{biy.}]{}{=}f\left( U \right)} \subset Y \Rightarrow f$ abierta.
\end{enumerate}
\end{demo}
...
%TODO: Corregir
\begin{demo}
Si $f$ es homeomorfismo local, entonces $\exists V^a \subset X$ y $V^{f\left( a \right)} \subset Y$, tal que: 
\[
    f: V^a \rightarrow V^{f\left( a \right)}
\]
Es homeomorfismo. Por esta razón, sabemos que $\exists U^a (\ab V^a) \mapsto f\left( U^a \right) (\ab V^{f\left( a \right)})$. 

Además, como $f$ es abierto local en $V^{f\left( a \right)}$ (homeomorfismo local), $f\left( U^a \right)$ es entorno de $f\left( a \right)$ (no tiene porque ser abierto ya que no sabemos si $f\left( U^a \right) \subset V^{f\left( a \right)}$). 

Por tanto, tiene un abierto ($W^{f\left( a \right)} \subset f\left( U \right)$) que es imagen de un abierto, $G \ab X$:
\[
X \stackrel{\text{ab.}}{\supset} G = W^a \rightarrow f\left( G \right) = W^{f\left( a \right)} \ab Y
\]
\end{demo}
\end{obs}
...
\end{prop}
\begin{demo}
    $U \ab X \xRightarrow{?} f\left( U \right)$ entorno $\forall y_0 = f\overbrace{\left( x_0 \right)}^{\in U} \in f\left( U \right)$.

    Como $f$ homeomorfismo local $\Rightarrow \forall x_0 \in U,\ \exists f| : V^{x_0} \rightarrow V^{y_0}$, homeomorfismo $\Rightarrow f\overbrace{\left( U \cap V^{x_0} \right)}^{\ni y_0 = f\left( x_0 \right)} \ab V^{y_0} \Rightarrow f\overbrace{\left( U \cap V^{x_0} \right)}^{\subset f\left( U \right)}$ entorno de $y_0 \Rightarrow f\left( U \right)$ entorno de $y_0$.

    Se puede ver también porque, como $U = \bigcup_{x \in U} W^x (\ab V^{x}) \Rightarrow f\left( U \right) = f\left( \bigcup_{x} W^{x} \right) = \bigcup_{x} f\left( W^x \right)$ es abierto por unión de abiertos.
\end{demo}
