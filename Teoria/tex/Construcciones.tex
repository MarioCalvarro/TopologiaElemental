...
\begin{theo}[Caracterización de imágenes directas]
...
Los conjuntos saturados nos van a permitir caracterizar los abiertos de la topología imagen directa. Sin embargo y de forma preeliminar, vamos a detenernos un instante a analizar qué significa que un conjunto sea saturado.
...
\begin{ej}
\begin{enumerate}
    \item Consideremos la aplicación que transforma el intervalo $[0,1]$ en la circunferencia unidad de $\mathbb{R}^2$ y que viene definida por la expresión:
    \begin{align*}
        f: \left[ 0, 1 \right] &\longrightarrow \mathbb{S}^1 \\
        t &\longmapsto \left( \cos 2\pi t, \sin 2\pi t \right)
    \end{align*}
    La transformación que realiza entre el intervalo y la circunferencia está representada en la figura \ref{fig:proyeccion_exponencial_img_directa}:
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            %Flechas y texto
    ...
    \end{figure}
    donde podemos observar dos comportamientos distintos: cualquier punto del intervalo $(0,1)$ tiene asignado un único punto de la circunferencia $\mathcal{S}^1$ y los puntos $0$ y $1$ tienen asignado el mismo punto $f(0) = f(1) = (1,0)$.
...
\begin{itemize}
    ...
    \begin{figure}[H]
        ...
        \end{tikzpicture}
        \caption{\textit{Ejemplos de identificaciones habituales.}}
    \end{figure}
\end{itemize}
\end{ej}
...
entonces $\mathcal{T}' = \prod_{i = 1}^{r} \mathcal{T}_i$ si y sólo si:
$$
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering    
            ...
            \node(Y) {$\left( Y, \mathcal{T}' \right)$};
            \node(X) [right of=Y] {$\left( X_i, \mathcal{T}_i \right)$};
            \node(Z) [below of=Y] {$\left( Y, \prod \mathcal{T}_i \right)$};
            \draw[->](Y) to node {$p_i$}(X);
...
\begin{ej}
Para ejemplificar que la topología producto no se da ``por defecto'', vamos a demostrar que la topología radial $\mathcal{T}_{\text{rad}}$ no es producto.

Para verlo, vamos a tomar como abierto el conjunto $U := \mathbb{R}^2 \setminus \mbox{arco}$ que se ve en la figura. 
...
\begin{figure}[H]
    ...
    \draw[dashed, red] (0,0) arc (270:360:1);
...
\begin{figure}[H]
    ...
    \draw[-, dashed] (0,0.4) -- (0.8, 0.4);
    \draw[-, dashed] (0.8,0) -- (0.8, 0.4);
    \node(O) at (0,0) {\textbullet};

    \node at (0,0.4)  {\tiny \textbullet};
    \node(U1) at (0,0.35) [left] {\tiny$y_1 \not\in U_1$};

    \node at (0.8,0)  {\tiny \textbullet};
    \node(U2) at (0.6,0) [below] {\tiny$U_1 \ni x_1$};
    \node at (0.8,0.4)  {\tiny \textbullet};
    \node(xy) at (0.8,0.4) [right] {\tiny$\left( x, y \right) \not\in U$};
    \end{axis}
    \end{tikzpicture}
    \caption{\textit{La topología radial no es producto de topologías.}}
\end{figure}
...
Sea $g: \left( Y, \mathcal{T}' \right) \rightarrow \left( Z, \mathcal{T}'' \right)$ y $j_i: \left( X_i, \mathcal{T}_i \right) \rightarrow \left( Y, \mathcal{T}' \right)$:
    \begin{figure}[H]
        \centering    
            ...
            \node(Y) [right of=X] {$\left( Y, \mathcal{T}' \right)$};
            \node(Z) [below of=Y] {$\left( Z, \mathcal{T}'' \right)$};
            \draw[->](X) to node {$j_i$}(Y);
            \draw[->](X) to node [left] {$g|_{X_i} = g \circ j_i$}(Z);
            \draw[->](Y) to node {$g$}(Z);
            \end{tikzpicture}
        \caption{\textit{Ilustración de la composición propuesta}}
...
\begin{figure}[H]
    \centering
    \incfig[0.5]{proyectivo,-disco-y-moebius}
